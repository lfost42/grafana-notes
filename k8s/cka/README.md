...
For lab practice, I spun up a local version of [sailor-sh](k8s/CKAplab.md) which is an exam simulator comparable to killer.sh. There were some things that wouldn't work and required a detour to a [Killercoda Playground](https://killercoda.com/playgrounds/scenario/cka). However, I like to work with CK-X whenever possible to get used to the interface. 

I am toying with the idea of creating a CKA set for sailor-sh but that will need to wait until my to-do list dwindles down a bit and I'm less constrained for time. 
...
Planned:
- [simonbbb](https://github.com/simonbbbb/CKA-Hand-on-lab) - This one creates the cluster so you'll be running this on your local terminal which is why I'm saving it for last. 
